...
class BinaryFile:
    def __init__(self, path, mode='rb', byteOrder='>', offset=0):
        if type(path) is str:
            self.path = path
            self.mode = mode
    ...
    def read(self, size, offset=None) -> bytes:
    ...
    def reads8(self, count=1, offset=None) -> int:
        return self._readFmt('%db' % count, offset)

    def readu8(self, count=1, offset=None) -> int:
        return self._readFmt('%dB' % count, offset)

    def readChar(self, count=1, offset=None) -> int:
        return self._readFmt('%dc' % count, offset)

    def reads16(self, count=1, offset=None) -> int:
        return self._readFmt('%dh' % count, offset)

    def readu16(self, count=1, offset=None) -> int:
        return self._readFmt('%dH' % count, offset)

    def reads32(self, count=1, offset=None) -> int:
        return self._readFmt('%di' % count, offset)

    def readu32(self, count=1, offset=None) -> int:
        return self._readFmt('%dI' % count, offset)

    def reads64(self, count=1, offset=None) -> int:
        return self._readFmt('%dq' % count, offset)

    def readu64(self, count=1, offset=None) -> int:
        return self._readFmt('%dQ' % count, offset)

    def readFloat(self, count=1, offset=None) -> float:
        return self._readFmt('%df' % count, offset)

    def readDouble(self, count=1, offset=None) -> float:
    ...
    def readString(self, offset=None, maxLen=1000000) -> str:
        r = []
        if offset is not None: self.seek(offset)
        while len(r) < maxLen:
            b = self.file.read(1)
    ...
    def readBytes(self, count=None, offset=None) -> bytes:
        if offset is not None: self.seek(offset)
